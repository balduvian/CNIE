...
std::function<void(HWND, HDC)> cnie::onDraw;
...
ATOM cnie::registerClass(WNDPROC proc, const WCHAR className[]) {
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_PHOTOBOOTH));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_PHOTOBOOTH);
...
int cnie::setup(
	...
	LoadStringW(hInstance, IDC_PHOTOBOOTH, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_PHOTOBOOTH));
